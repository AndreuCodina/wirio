...
from aspy_dependency_injection.types import AnnotatedParameter, InjectableType
...
def ensure_is_type(
    value: type[T] | str, globalns: dict[str, Any] | None = None
) -> type[T] | None:
    ...
    if isinstance(value, str):
        try:
            ...
            return eval_type_backport.eval_type_backport(  # type:ignore[no-any-return]
                eval_type_backport.ForwardRef(value),
                globalns=globalns,
                try_default=False,
            )
        except NameError:
...
def _get_injectable_type(metadata: Any) -> InjectableType | None:
    ...
    if hasattr(metadata, "dependency") and hasattr(
        metadata.dependency, "__is_wireup_depends__"
    ):
...
def param_get_annotation(
    parameter: Parameter, *, globalns: dict[str, Any]
    ...
    if hasattr(parameter.annotation, "__metadata__") and hasattr(
        parameter.annotation, "__args__"
    ):
        is_aspy_dependency = get_aspy_dependency(parameter.annotation.__metadata__)
    ...
    # TODO: Resolve Annotated[Optional[T]]
    # else:
...
def get_aspy_dependency(metadata: Sequence[Any]) -> InjectableType | None:
    """Check if the given metadata contains an aspy dependency."""
    for meta in metadata:
        if hasattr(meta, "__is_aspy_depends__"):
            return meta
